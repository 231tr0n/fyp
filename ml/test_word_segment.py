...
with mp_hands.Hands(
    ...
    while cap.isOpened():
        ...
        if results.multi_hand_landmarks:
            ...
            res = list(filter(lambda i: i > 0.5, plist[0]))
            if len(res) > 0:
            ...
            if underscore_counter >= 20:
                temp_sentence = ''
                underscore_counter = 0
                print("Temp sentence cleared")
            if len(temp_sentence) == 30:
                temp1 = Counter(temp_sentence)
                temp1 = max(temp1, key = temp1.get)
                sentence += str(temp1)
                temp_sentence = ''
                print("Temp sentence:", sentence)
            else:
                if prediction != "_":
                    temp_sentence += prediction
        else:
            underscore_counter += 1
        ...
        if cv2.waitKey(1) & 0xFF == 27:
            break
    corrected_sentence = wordsegment.segment(sentence)
    print("Correct word:", corrected_sentence)
